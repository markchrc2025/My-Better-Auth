import { useState } from "react";

/**
 * When an OAuth authorize request is rejected before a client redirect is
 * possible (unknown client, invalid redirect_uri, ...), the provider lands
 * the browser on this app with ?error=...&error_description=... — surface it
 * loudly instead of silently showing the page.
 */
const HINTS: Record<string, string> = {
  invalid_redirect:
    "This app's callback URL isn't registered. Open the app under Applications and add the exact URL below to its Redirect URIs.",
  invalid_client:
    "Unknown or disabled client_id — check the application's credentials and enabled state.",
  client_disabled: "This application is disabled. Enable it on its detail page.",
  management_account:
    "This is a platform management account — it administers Authenticize and cannot sign into connected applications. Sign in with an application account instead.",
  signup_disabled:
    "That Google/Microsoft account hasn't been invited. Authenticize is invite-only: an administrator has to add your email here first, then this sign-in will link to it.",
  account_not_linked:
    "We found an account with that email but couldn't link this social login to it. Ask an administrator to check your account, or sign in with your email and password.",
  email_not_found:
    "The provider didn't share an email address, so we can't match you to an account. Try a different sign-in method.",
};

const TITLES: Record<string, string> = {
  management_account: "Management accounts can't sign into apps",
  signup_disabled: "This account isn't invited yet",
  account_not_linked: "Couldn't link this social login",
  invalid_redirect: "That callback URL isn't registered",
};

export function OAuthErrorNotice() {
  const [dismissed, setDismissed] = useState(false);
  const params = new URLSearchParams(window.location.search);
  const error = params.get("error");
  if (!error || dismissed) return null;
  const description = params.get("error_description");
  const attempted = params.get("attempted");

  return (
    <div className="mb-6 rounded-lg border border-red-900/60 bg-red-950/40 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-semibold text-red-200">
            {TITLES[error] ?? (
              <>
                An app's sign-in request was rejected:{" "}
                <span className="code">{error}</span>
              </>
            )}
          </div>
          {description && (
            <p className="mt-1 text-sm text-red-300/90">{description}</p>
          )}
          {HINTS[error] && <p className="mt-2 text-xs text-red-300/80">{HINTS[error]}</p>}
          {error === "invalid_redirect" && attempted && (
            <code className="mt-2 block overflow-x-auto whitespace-nowrap rounded border border-red-900/60 bg-red-950/70 px-2 py-1.5 font-mono text-xs text-red-100">
              {attempted}
            </code>
          )}
        </div>
        <button
          className="btn-ghost -mr-1 px-2 py-1 text-red-300"
          onClick={() => setDismissed(true)}
          aria-label="Dismiss"
        >
          ✕
        </button>
      </div>
    </div>
  );
}
